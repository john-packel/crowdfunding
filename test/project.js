...
contract('Project', function(accounts) {
    ...
    let project;
    const deadlineTime = (new Date().getTime() / 1000) + 5;

    beforeEach(async () => {
      project = await Project.new(fromAscii("Test Project"), accounts[1], 10000, deadlineTime, { from: accounts[0] });
    });

    it("should be able to fund a project", async () => {
        await project.fund(accounts[2], {from: accounts[0], value: 5000, gas: 300000});
    });
    //
    // it("should not be able to fund a from non-hub address", async () => {
